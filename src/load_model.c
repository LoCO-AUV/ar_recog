...
ObjectData_T *read_ObjData( char *name, int *objectnum )
{
    FILE          *fp;
    ObjectData_T  *object;
    char           buf[256], buf1[256], model_name[256], fullname[512];
    int            i;
	...
	sprintf(fullname,"%s/%s",dirname(model_name),buf1);
	printf("Reading pattern: %s",fullname);
        if( (object[i].id = arLoadPatt(fullname)) < 0 )
            {fclose(fp); free(object); return(0);}
